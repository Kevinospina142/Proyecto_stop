...
int main() {
  ...
  int npl, pts[6], hit[6], miss[6], chro[6];
  int ptst=0, hitt=0, misst=0, chrot=0;
  string name[6], nom[6], ape[6], ciu[6], col[6], cos[6];
  char con, vDatos[30];
  

  for(int i=1;i<=5;i++){//limpia los arreglos, algunos tienen residuos
      pts[i] = 0;
      hit[i] = 0;
      miss[i] = 0;
      chro[i] = 0;
...
cout << "3) Con cada acierto obtienes 50 puntos, con cada error pierdes 50 puntos"<<endl;
  cout << "4) Si no sabes que escribir, "<<endl;
/////////////////////////////////////////////////////////////////////////////////////
  ...
  if (con == 's'){
    tti = time(0);
    cout << "Bienvenido(s), cuantos jugadores desean jugar (maximo 5): ";
    cin >> npl;
    cout << endl;
